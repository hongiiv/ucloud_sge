...
subj="[320fbe17-4f73-4be7-8a51-36ee80c93a84] AWS Message Services - Master Node"
res=sns.publish(mytopic_arn,msg,subj)
EOF

echo "deploy product from script" >> /tmp/deploy_result.txt
echo "master ip: 172.27.223.71" >> /tmp/deploy_result.txt
echo "slave ip: dS9itnhkm" >> /tmp/deploy_result.txt
