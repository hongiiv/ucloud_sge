...
def update_db_product(productid):
   ...
   if status == "1":
      ...
      if deploy_result == "1":
         ...
         log.debug("Disk attach fail: %s"%(product_id))
   elif status == "2":
...
import time
while True:
   command_external = os.system("/usr/bin/wget ftp://%s/%s_master.sh -O /tmp/%s_master.sh")
   if os.path.isfile("/tmp/%s_master.sh") and os.path.getsize("tmp/%s_master.sh") > 0:
      command_external = os.system("/bin/bash /tmp/%s_master.sh")
...
'''%(deploy_server_ip, message_data['clusteruuid'], message_data['clusteruuid'], message_data['clusteruuid'], message_data['clusteruuid'], message_data['clusteruuid'])
   ...
   if message_data['vm_tot_count'] == '1':
